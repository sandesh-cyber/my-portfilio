// ------------------------------------------------
// Project Name: sandesh garde- Personal Portfolio & Resume HTML Template
// Project Description: Show yourself brightly with sandesh garde - unique and creative portfolio and resume template!
...
$(function() {
  ...
  let lastScrollTop = 0; 
  let rotations = {}; // Store individual rotations
  
  window.addEventListener("scroll", function () {
    let scrollTop = window.scrollY;
  
    document.querySelectorAll(".animate-rotation").forEach((svg) => {
      let id = svg.getAttribute("data-id") || "default"; // Unique ID for each SVG
  
      if (!rotations[id]) rotations[id] = 0; // Initialize rotation if not set
  
      if (scrollTop > lastScrollTop) {
        rotations[id] += 0.5; // Rotate right when scrolling down
      } else {
        rotations[id] -= 0.5; // Rotate left when scrolling up
      }
  
      svg.style.transform = `rotate(${rotations[id]}deg)`;
    });
